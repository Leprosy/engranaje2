...
class Eng_PageController {
	...
	public function hasAction($actionName) {
		if (method_exists($this, Eng_Config::getActionFunctionName($actionName))) {
			return true;
		} else {
			return false;
		}
	...
	public function runAction($actionName, &$actionParams = array()) {
		...
		$func = Eng_Config::getActionFunctionName($actionName);
		if (method_exists($this, $func)) {
			$this->beforeAction();
			
			// Call the action
			if (Eng_Config::getSendActionParamsAsArray()) {
			...
			throw new Eng_Exception('No action `' . $actionName . '`. Write the `' . $func . '` method');
		}
	}
	...
	protected function loadView($controllerViewName) {
		...
		$view = Eng_Config::getControllerView($controllerViewName, $this->viewVars);
		if (is_null($view)) {
			throw new Eng_Exception('Unable to load controller view "' . $controllerViewName . '" for controller "' . $this->controllerName . '"');
		} else {
		...
		if ( ! empty($this->layout)) {
			...
			$this->layoutVars[Eng_Config::getLayoutContentVarName()] = $viewContents;
			$layoutView = Eng_Config::getLayoutView($this->layout, $this->layoutVars);
			if (is_null($layoutView)) {
				throw new Eng_Exception('Unable to load layout view "' . $this->layout . '" for controller "' . $this->controllerName . '"');
			} else {
